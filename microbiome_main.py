...
def multicomp_decision(tcdb_proteins, tcid, mem_dict, tc_filter_arr):
    ...
    if 'Q52978' in tc_filter_arr:
       print((len(tc_filter_arr)/len(tcdb_proteins)))
...
def isMultiComp(row,df,input):
    ...
    decision = multicomp_decision(tc_arr, row['Hit_tcid'], MembraneProteins, tc_filter_arr)
    if row['Hit_tcid'] == '2.A.63.1.1':
        print(decision)
        print(len(set(tc_filter_arr)))
    if len(tc_missing_arr) == 0 and final_decision(eVal(row), qCoverage(row), hCoverage(row), pfamDoms(row), sortedGeneArr, overlap_percent(row), tcdb_tms) == 'yellow':
        return({"color":"Yellow",
                "Found_proteins":tc_filter_arr,
                "All_proteins":tc_arr,
                'Missing_proteins':tc_missing_arr,
    ...
    if (total_tcmem > 0 and input <= float(count_mem / total_tcmem)) or (len(set(tc_filter_arr))>0 and isEmpty(MembraneProteins)):
        ...
        if final_decision(eVal(row), qCoverage(row), hCoverage(row), pfamDoms(row), sortedGeneArr, overlap_percent(row), tcdb_tms) == 'yellow' or final_decision(eVal(row), qCoverage(row), hCoverage(row), pfamDoms(row), sortedGeneArr, overlap_percent(row), tcdb_tms) == 'green': 
            if decision == 'yellow':
                return({"color":"Yellow",
                        "Found_proteins":tc_filter_arr,
                        "All_proteins":tc_arr,
